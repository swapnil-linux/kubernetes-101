...
```
$ kubectl create serviceaccount newsa1
...
```
$ kubectl run hello-1 --image=quay.io/mask365/scaling 
pod/hello-1 created
```
...
```
$ cat ~/kubernetes-101/labs/security/pod.yaml
apiVersion: v1
kind: Pod
metadata:
...
```
$ kubectl create -f ~/kubernetes-101/labs/security/pod.yaml
pod/hello-2 created
```
...
```
$ kubectl exec hello-1 -- cat /var/run/secrets/kubernetes.io/serviceaccount/token
...
```
$ kubectl exec hello-2 -- cat /var/run/secrets/kubernetes.io/serviceaccount/token
...
```
bash-5.1# curl -k -H "Authorization: Bearer $(cat /var/run/secrets/kubernetes.io/serviceaccount/token)" https://kubernetes.default.svc.cluster.local/api/v1
{
...
```
$ cat kubernetes-101/labs/security/clusterrole-list-pods.yml
...
```
$ kubectl create -f kubernetes-101/labs/security/clusterrole-list-pods.yml
...
```
$ cat ~/kubernetes-101/labs/security/clusterrolebinding.yml
apiVersion: rbac.authorization.k8s.io/v1
...
```
$ kubectl create -f ~/kubernetes-101/labs/security/clusterrolebinding.yml
clusterrolebinding.rbac.authorization.k8s.io/list-pods created
```

```
$ kubectl get clusterrolebindings list-pods -o wide
