...
```
[centos@ip-10-0-2-94 ~]$ kubectl expose deployment scaling --type=NodePort 
service/scaling exposed
```

```
[centos@ip-10-0-2-94 ~]$ kubectl get svc -o wide
...
```
[centos@ip-10-0-2-94 ~]$ curl http://localhost:30298
...
```
[centos@ip-10-0-2-94 ~]$ while sleep 1; do curl http://localhost:`kubectl get svc -o yaml|grep nodePort|awk '{print $3}'`; done
...
```
[centos@ip-10-0-2-94 ~]$ kubectl scale deployment scaling --replicas=5
...
```
[centos@ip-10-0-2-94 ~]$ kubectl edit deployments.apps scaling 
...
```
[centos@ip-10-0-2-94 ~]$ kubectl rollout status deployment scaling                                                              
...
```
[centos@ip-10-0-2-94 ~]$ kubectl get rs
...
```
[centos@ip-10-0-2-94 ~]$ kubectl describe rs scaling-5cf66b5fcc
...
```
[centos@ip-10-0-2-94 ~]$ kubectl rollout undo deployment scaling --to-revision=1 ; kubectl rollout status deployment scaling
